...
<input type="number" step="0.01" name="prix_bebe" class="form-control mb-2"
    ...
    <?= (!empty($voyageToEdit['id_destination']) && (int)$voyageToEdit['id_destination'] === (int)$d['id_destination']) ? 'selected' : '' ?>>
    <?= htmlspecialchars($d['nom']) ?> — <?= htmlspecialchars($d['continent_nom'] ?? '') ?>
</option>
    ...
    <?php if (!empty($voyageToEdit['image'])): ?>
        <p>Image actuelle :</p>
        <img src="images/voyages/<?= htmlspecialchars((string)$voyageToEdit['image']) ?>" width="150" style="border:1px solid #ccc; padding:2px;">
    <?php endif; ?>
    ...
    <tbody>
        <?php if (!empty($voyages)): ?>
            ...
            <tr>
                ...
                <td><?= htmlspecialchars((string)($v['prix'] ?? '')) ?></td>
