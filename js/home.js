...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (dateDepart && dateArrivee && dateSuggestion) {
        ...
        window.updateDateSuggestion = function () {
            const format = d => d ? new Date(d).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' }) : '';
            const text = dateDepart.value && dateArrivee.value
                ? `${format(dateDepart.value)} - ${format(dateArrivee.value)}`
                : dateDepart.value ? format(dateDepart.value) : dateArrivee.value ? format(dateArrivee.value) : '';
