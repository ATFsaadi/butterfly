...
class Modele
{
    ...
    public function insertVoyage(array $tab): void
    {
        $sql = "INSERT INTO voyages
                (titre, prix_adulte, prix_enfant, prix_bebe, date_depart, date_retour, description, image, id_destination)
                VALUES
                (:titre, :prix_adulte, :prix_enfant, :prix_bebe, :date_depart, :date_retour, :description, :image, :id_destination)";
    ...
    public function insertOffre(array $tab): void
    {
        $sql = "INSERT INTO offres
                (titre, reduction, date_debut, date_fin, description, actif, id_voyage)
                VALUES
                (:titre, :reduction, :date_debut, :date_fin, :description, :actif, :id_voyage)";
    ...
    public function addReservation(array $tab): int|false
    {
        $sql = "INSERT INTO reservations
                ...
                VALUES
                (:id_utilisateur, :id_voyage, :date_depart, :date_retour, :adultes, :enfants, :bebes, :prix_total)";
    ...
    {
        $sql = "UPDATE reservations SET paiement_statut = 'payé' WHERE id_reservation = :id";
        $stmt = $this->unPdo->prepare($sql);
        return $stmt->execute([':id' => $id]);
    }
    ...
    {
        $sql = "SELECT COUNT(*) FROM reservations WHERE statut = 'en attente'";
        return (int) $this->unPdo->query($sql)->fetchColumn();
    }
}
