...
class Modele
{
    ...
    public function selectAll_reservations_voyages(): array
    {
        $sql = "select rv.*,
                       ...
                       v.prix as voyage_prix,
                       v.statut as voyage_statut,
                       d.pays, d.ville,
                       cont.nom as continent
    ...
    public function getStatsDashboardAdmin(): array
    {
        ...
        $totalReservations =
            (int) ($resaDest["total_resa_dest"] ?? 0)
            + (int) ($resaVoy["total_resa_voy"] ?? 0);

        $chiffreAffaires =
            (float) ($resaDest["total_ca_dest"] ?? 0)
            + (float) ($resaVoy["total_ca_voy"] ?? 0);
